...
const ReservationLookup = () => {
    ...
    const { getReservationByCode, getReservationsByPhone, loading } = useReservation();
    ...
    const handlePhoneSubmit = async (e) => {
        ...
        try {
            ...
            if (reservations && reservations.length > 0) {
                // Navigate to the reservations page with the phone number in URL
                navigate(`/reservations/${encodeURIComponent(phoneNumber)}`);
            } else {
                setError('No reservations found for this phone number. Please check the number and try again.');
                setSubmitted(false);
            }
        } catch (err) {
            setError('Failed to find reservations. Please check your phone number and try again.');
                    ...
                    {searchType === 'phone' && (
                        <form onSubmit={handlePhoneSubmit} className="space-y-6">
                            ...
                            <div>
                                <button
                                    type="submit"
                                    disabled={loading || !phoneNumber || submitted}
                                    className="w-full flex justify-center items-center py-3 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-green-600 hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 disabled:opacity-50"
                                >
                                    {loading || submitted ? (
                                        <>
