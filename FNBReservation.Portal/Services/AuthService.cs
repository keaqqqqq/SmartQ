...
namespace FNBReservation.Portal.Services
{
    public class AuthService
    {
            ...
            JwtTokenService tokenService = null)
        {
            ...
            _refreshTokenEndpoint = _configuration["ApiSettings:AuthEndpoints:RefreshToken"] ?? "api/v1/auth/refresh-token";
        }

        public async Task<bool> LoginAsync(string username, string password)
        {
            return await LoginAsync(username, password, true);
        }

        public async Task<bool> LoginAsync(string username, string password, bool rememberMe)
        {
            var result = await Login(username, password, rememberMe);
        ...
        public async Task RefreshToken()
        {
            try
            {
                ...
                {
                    // For cookie-based authentication, we might just check if the cookie is still valid
                    // This is a simple implementation
                }

                // For JWT tokens, we would call an API endpoint to refresh the token
                if (_tokenService != null)
                {
            ...
            {
                await _jsRuntime.InvokeVoidAsync("console.log", "RefreshToken exception: " + ex.Message);
            }
        }
